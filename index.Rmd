...
|----------------|------------------------|----------------|----------------|
| data_dictionary_AFINN            | Nielsen's (2011) 'new ANEW' valenced word list                |              | X           |
| data_dictionary_ANEW             | Affective Norms for English Words (ANEW)                      |              | X           |
...
We can apply these established dictionaries to larger and more complex corpora. In the following example, we'll examine trends in the sentiment of immigration tweets from 2013-2017. 
...
We can manipulate the dataframe using tidyverse to examine the proportion of words that represent positive versus negative sentiment by each year. [AMBER: Might want to add description of sentiment score?] In the code below, we manipulate the `tweets_analysis` df to calculate percent positive, percent negative, and the difference (positive - negative) for a sentiment score for each year.
...
We can then plot the percent positive sentiment words using `ggplot2`. The figure below illustrates that tweets on immigration have become decreasingly positive since 2013, as indicated by the downward trend. 
...
**Question 2: Replicate the immigration tweet example from above, but this time apply the sentiment analysis skills to the Trump tweets corpus we previously analyzed ("trump_tweet_corp.rds"). Compare the results of sentiment analysis to the word frequency approaches and describe any differences in the trends observed.**
...
eu+topics_pa = convert(dfm_eu_pa, "data.frame") %>%
